...
{
	// FTYPE *A, *B;
	// A = (FTYPE *)_mm_malloc(sizeof(FTYPE)*n*middle,64);
	// B = (FTYPE *)_mm_malloc(sizeof(FTYPE)*nr*middle,64);
	...
	// #pragma omp parallel for
	// for(int i=0;i<nr*middle;i++) {
	// 	B[i] = 1;
	// }

	// col_idx = (typeof(col_idx))__builtin_assume_aligned(col_idx, 64);	// __assume_aligned(col_idx, 64);
	// val = (typeof(val))__builtin_assume_aligned(val, 64);	// __assume_aligned(val, 64);
	// val_out = (typeof(val_out))__builtin_assume_aligned(val_out, 64);	// __assume_aligned(val_out, 64);
	// A = (typeof(A))__builtin_assume_aligned(A, 64);	// __assume_aligned(A, 64);
	// B = (typeof(B))__builtin_assume_aligned(B, 64);	// __assume_aligned(B, 64);
					...
					for(int i=row_panel*BH; i<(row_panel+1)*BH; i++) {
						...
						for(j=loc1; j<interm; j+=8) {
							...
							for(int k=0; k<middle; k++) {
								val_out[j] = val_out[j] + A[col_idx[j]*middle + k] * B[i*middle + k];
								val_out[j+1] = val_out[j+1] + A[col_idx[j+1]*middle + k] * B[i*middle + k];
								val_out[j+2] = val_out[j+2] + A[col_idx[j+2]*middle + k] * B[i*middle + k];
								val_out[j+3] = val_out[j+3] + A[col_idx[j+3]*middle + k] * B[i*middle + k];
								val_out[j+4] = val_out[j+4] + A[col_idx[j+4]*middle + k] * B[i*middle + k];
								val_out[j+5] = val_out[j+5] + A[col_idx[j+5]*middle + k] * B[i*middle + k];
								val_out[j+6] = val_out[j+6] + A[col_idx[j+6]*middle + k] * B[i*middle + k];
								val_out[j+7] = val_out[j+7] + A[col_idx[j+7]*middle + k] * B[i*middle + k];
							}
							// #pragma ivdep
							for(int k=0; k<8; k++) {
								val_out[j+k] *= val[j+k];
