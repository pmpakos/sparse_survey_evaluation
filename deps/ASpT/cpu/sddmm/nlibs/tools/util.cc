...
QValue arraySum(const QValue *__restrict values, const int count) {
  QValue rsum = 0.0;
  for (int i = 0; i < count; ++i) {
    rsum += values[i];
  }
...
void arrayInflationR2(const QValue *__restrict ivalues, const int count, QValue *__restrict ovalues) {
  for (int i = 0; i < count; ++i) {
