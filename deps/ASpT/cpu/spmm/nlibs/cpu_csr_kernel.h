...
//inline int indexProcessCRowI(int *__restrict index, // index array must be initilized with -1
#ifdef __CUDACC__
inline int indexProcessCRowI(int *index,
    ...
    int* iJC, QValue* iC) {
#else
inline int indexProcessCRowI(int *__restrict index,
    const int iAnnz, const int iJA[], const QValue iA[],
    const int IB[], const int JB[], const QValue B[],
    int* __restrict iJC, QValue* __restrict iC) {
#endif
  if (iAnnz == 0) {
    return 0;
  }
