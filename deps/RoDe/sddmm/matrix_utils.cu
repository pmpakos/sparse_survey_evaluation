...
SparseMatrix::SparseMatrix(int * row_ptr, int * col_idx, float * val, int m, int n, int nnz, Swizzle row_swizzle, int pad_rows_to) 
{
  row_swizzle_ = row_swizzle;
  pad_rows_to_ = pad_rows_to;
  ...
  for(int i=0; i<nnz ; i++) 
  {
    column_indices_staging[i] = col_idx[i];
    values_staging[i] = val[i];
  }
  for(int i=0; i< m+1 ;i++) {
    row_offsets_staging[i] = row_ptr[i];
  }
