        ...
        : Matrix_Format(m, n, nnz, k)
    {
        ...
        time_balance = time_it(1,
            _Pragma("omp parallel")
            {
                ...
                #ifdef CUSTOM_COO_VEC_FRONTIER_RINGBUFFER
...
void COOArrays::preprocess_frontier_ringbuffer(int r_first_rows, int cross_distance_limit, int frontier_size, int cache_capacity, float alpha, float beta) 
{
    #pragma omp parallel
    {
        ...
        if (local_nnz <= 0) printf("Thread %d has no work (nnz range [%ld, %ld))\n", tnum, j_s, j_e);
        ...
        // Optimization: Use flat arrays for counts since row/col indices are bounded by M and N
        ...
        // 2. Find Seeds (Using the flat array)
        ...
        // The Ring Buffers (Size based on physical hardware capacity)
        ...
        auto compute_score = [&](long j) -> double {
            ...
            // R(v): Recency is simply a binary check of the flat arrays!
            double r_score = (row_in_cache[r] ? 1.0 : 0.0) + (col_in_cache[c] ? 1.0 : 0.0);

            // N(v): Coverage uses flat arrays (no hashing)
            double n_score = 0.0;
        ...
        while (reordered_j.size() < local_nnz) {
            ...
            if (evict_r != -1) row_in_cache[evict_r] = 0; // Evict
            row_history[r_head] = best_r;                 // Insert to History
            row_in_cache[best_r] = 1;                     // Mark active
            r_head = (r_head + 1) % cache_capacity;       // Move Head
            
            // Col Cache
            long evict_c = col_history[c_head];
            if (evict_c != -1) col_in_cache[evict_c] = 0; // Evict
            col_history[c_head] = best_c;                 // Insert to History
            col_in_cache[best_c] = 1;                     // Mark active
            c_head = (c_head + 1) % cache_capacity;       // Move Head
...
csr_to_format(INT_T * row_ptr, INT_T * col_ind, ValueType * values, long m, long n, long nnz, int k)
{
    ...
    // printf("Running COO Vector Cross Kernel with %d threads...\n", omp_get_max_threads());
    coo->format_name = (char *) "COO_Frontier_Ringbuffer";
    return coo;
}
