...
INT_T * thread_min_r = NULL;
INT_T * thread_max_r = NULL;
...
struct COOZSegmentedArrays : Matrix_Format
{
    ...
    COOZSegmentedArrays(INT_T * csr_ia, INT_T * csr_ja, ValueType * csr_a, long m, long n, long nnz, int k) : Matrix_Format(m, n, nnz, k)
    {
        ...
        #pragma omp parallel
        {
            ...
            // Store min and max row for this thread (for initialization later)
            thread_min_r[tnum] = row_ind[thread_j_s[tnum]];
            thread_max_r[tnum] = row_ind[thread_j_e[tnum] - 1];
        }
...
COOZSegmentedArrays::spmm(ValueType * x, ValueType * y, int k)
{
    ...
    #pragma omp parallel
    {
        ...
        //     }
        // }
        long min_r = row_ind[j_s];
        long max_r = row_ind[j_e - 1];
...
csr_to_format(INT_T * row_ptr, INT_T * col_ind, ValueType * values, long m, long n, long nnz, int k)
{
    ...
    coo->format_name = (char *) "COO_Zorder_New";
    return coo;
}
